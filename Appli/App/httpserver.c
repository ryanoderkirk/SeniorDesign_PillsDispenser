...
char example_log_response[] = {
    ...
    "\r\n"
    "{\"logs\":[{\"category\":\"dispense\",\"time\":\"4PM\",\"channels\":\"1,2,"
    "3,4\"},{\"category\":\"containerOpened\",\"time\":\"4PM\",\"channels\":"
    "\"1\"},{\"category\":\"missedDispense\",\"time\":\"4PM\",\"channels\":"
    "\"2\"}]}"};
...
char example_put_response[] = {
    ...
    "Content-Type: application/json\r\n"
    "\r\n"
    "{\"logput\":[{\"category\":\"dispense\",\"time\":\"4PM\",\"channels\":\"1,"
    "2,3,4\"},{\"category\":\"containerOpened\",\"time\":\"4PM\",\"channels\":"
    "\"1\"},{\"category\":\"missedDispense\",\"time\":\"4PM\",\"channels\":"
    "\"2\"}]}"};
 ...
 */
static int32_t http_server_write(int32_t client, const char *buffer,
                                 size_t buffer_size);
...
void http_server_socket(void *arg) {
  ...
  if (W6X_Net_Setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout,
                         sizeof(timeout)) != 0) {
  ...
  if (W6X_Net_Setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout,
                         sizeof(timeout)) != 0) {
    LogError("Could not set the socket options.\n");
    goto _err;
  }
  ...
  /* Set the socket Send timeout option */
  if (W6X_Net_Setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (void *)&timeout,
                         sizeof(timeout)) != 0) {
    LogError("Could not set the socket options.\n");
    goto _err;
  }
  ...
  if (W6X_Net_Bind(sock, (struct sockaddr *)&s_addr_in_t,
                   sizeof(s_addr_in_t)) != 0) {
    LogInfo("\n LwIP Bind Fail\n");
    goto _err;
  }
  ...
  while (1) {
    ...
    int32_t newconn = W6X_Net_Accept(sock, (struct sockaddr *)&remotehost_t,
    ...
    if (newconn < 0) {
      ...
      LogDebug("\n Creation of temporary thread to process an incoming HTTP "
               "request : %" PRIi32 "\n",
               newconn);
      if (pdPASS != xTaskCreate((TaskFunction_t)http_server_serve_task,
                                thread_name, HTTP_CHILD_TASK_STACK_SIZE >> 2,
...
int parseDoseJSON(const char *json, Dosage_t *doses, int size) {
  ...
  for (int d = 0; d < count_to_parse; d++) {
    if (t[token_idx].type != JSMN_OBJECT)
      return d; // Stop if not an object
    ...
    for (int k = 0; k < obj_fields; k++) {
      ...
      if (jsoneq(json, &t[token_idx], "hour") == 0) {
        doses[d].hour =
            (uint8_t)strtol(json + t[token_idx + 1].start, NULL, 10);
...
void build_http_200_response(char *output_buffer, size_t max_len,
                             const char *body) {
 ...
 */
void build_http_error_response(char *output_buffer, size_t max_len,
...
static void http_server_serve_task(void *arg) {
  ...
  recv_buffer[0] = '\0';
  do {
    bytes_received = W6X_Net_Recv(
        client, (uint8_t *)&recv_buffer[recv_total_len], recv_buffer_len, 0);
...
// call flash function to get most recent log. Add full response to
...
static int get_time() {
  ...
  snprintf(response_body, sizeof(response_body),
           "20%02d-%02d-%02d %02d:%02d:%02d", date.Year, date.Month, date.Date,
...
static int set_doses(char *recv_buffer) {
  Dosage_t doses[5];
  char *body = strstr(recv_buffer, "\r\n\r\n");
  if (!body) {
    build_http_error_response(full_response, sizeof(full_response), 400,
...
static void http_process_response(int32_t client, char *recv_buffer) {
  ...
  for (uint32_t i = 0;
       i < sizeof(http_server_responses) / sizeof(http_server_responses[0]);
       i++) {
    if (strncmp(recv_buffer, http_server_responses[i].request,
                strlen(http_server_responses[i].request)) == 0) {
  ...
  if (response ==
      UNKNOWN_RESPONSE) /* Request not recognized, return 404 error */
  {
