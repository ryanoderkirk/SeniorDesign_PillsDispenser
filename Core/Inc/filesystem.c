...
int block_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off,
  ...
  if (osMutexAcquire(*flashMutex, 100) == osOK) {
    ...
    // 4. Send the 4-byte command/address packet and catch incoming garbage
    // bytes
    uint8_t dummy_rx[4];
    if (HAL_SPI_TransmitReceive(hspi, cmd, dummy_rx, 4, 1000) != HAL_OK) {
      HAL_GPIO_WritePin(Flash_CS_GPIO_Port, Flash_CS_Pin, GPIO_PIN_SET);
    ...
    if ((err = HAL_SPI_Receive(hspi, (uint8_t *)buffer, size, 1000)) !=
        HAL_OK) {
      HAL_GPIO_WritePin(Flash_CS_GPIO_Port, Flash_CS_Pin, GPIO_PIN_SET);
      osMutexRelease(*flashMutex);
      return LFS_ERR_IO;
...
int(block_program)(const struct lfs_config *c, lfs_block_t block, lfs_off_t off,
  ...
  if (osMutexAcquire(*flashMutex, 1000) == osOK) {
    ...
    HAL_GPIO_WritePin(Flash_CS_GPIO_Port, Flash_CS_Pin, GPIO_PIN_RESET);
    uint8_t dummy_wren;
    if (HAL_SPI_TransmitReceive(hspi, &wrenCmd, &dummy_wren, 1, 1000) !=
        HAL_OK) {
      HAL_GPIO_WritePin(Flash_CS_GPIO_Port, Flash_CS_Pin, GPIO_PIN_SET);
      osMutexRelease(*flashMutex);
      return LFS_ERR_IO;
    ...
    // Use a dummy Rx buffer to discard incoming garbage during data
    // transmission Using a VLA or max buffer of 256
    uint8_t dummy_payload[256];
    if (HAL_SPI_TransmitReceive(hspi, (uint8_t *)buffer, dummy_payload, size,
                                1000) != HAL_OK) {
      HAL_GPIO_WritePin(Flash_CS_GPIO_Port, Flash_CS_Pin, GPIO_PIN_SET);
      osMutexRelease(*flashMutex);
      return LFS_ERR_IO;
