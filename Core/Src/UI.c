...
const menuPage_t mainPage = {
    ...
    .backState = MAIN,
    .numOptions = 2,
    .options = {{.next = CONFIG, .name = "CONFIG"},
...
const menuPage_t configPage = {
    ...
    .backState = MAIN,
    .numOptions = 3,
    .options = {{.next = VERIFYPIN, .name = "CHANGE PIN"},
...
// const menuPage_t *menus[] =
// {&mainPage,&configPage,&schedulePage,&dispensePage};
...
void drawScreen() {
  ...
  case DOSAGELIST:
    ...
    for (int i = 0; i < numDosages; i++) {
      ILI9341_DrawText(dosageList[i].name, FONT3, 25,
                       i * font3Height + initialOffset, BLACK, WHITE);
    }
    drawCursor();
    break;
  case DOSAGEINFO:
    ...
    ILI9341_DrawText(doseTime, FONT4, 25, font3Height + initialOffset, BLACK,
                     WHITE);
    char pillAmount[32];
    char *pillNames[4] = {dose->p1, dose->p2, dose->p3, dose->p4};
    for (int i = 0; i < 4; ++i) {
      snprintf(pillAmount, 32, "%.10s: %02d", pillNames[i],
               dose->pillAmounts[i]);
      ILI9341_DrawText(pillAmount, FONT4, 25,
                       (1 + i) * font3Height + initialOffset, BLACK, WHITE);
    }
    break;
  case VERIFYPIN:
  case CHANGEPIN:
  ...
  case DISPENSE:
    ...
    ILI9341_DrawText(dosageList[selectedDosage].name, FONT3, 25, 25, BLACK,
                     WHITE);
    break;
  case SHOWTIME:
    char *time = getTime();
  ...
  case DISPENSE_ALERT:
    ...
    char timeStr[10] = "  :  ";
    snprintf(timeStr, 10, "%02d:%02d", dosage.hour, dosage.min);
    char DAbuffer1[32];
    snprintf(DAbuffer1, 32, "Dosage at %s", timeStr);
    ILI9341_DrawText(DAbuffer1, FONT3, 10, 25, BLACK, WHITE);
    uint8_t pillAmounts[4] = {dosage.pillOneCount, dosage.pillTwoCount,
                              dosage.pillThreeCount, dosage.pillFourCount};
    uint8_t *names[4] = {dosage.pillOne, dosage.pillTwo, dosage.pillThree,
                         dosage.pillFour};
    for (int i = 0; i < 4; ++i) {
      char DAbuffer2[32];
      snprintf(DAbuffer2, 32, "%.10s: %02d", names[i], pillAmounts[i]);
      ILI9341_DrawText(DAbuffer2, FONT3, 10, 25 + (i + 1) * font3Height, BLACK,
                       WHITE);
    }
...
void drawMenuPage(const menuPage_t *this) {
  ...
  for (int i = 0; i < this->numOptions; i++) {
    ILI9341_DrawText(this->options[i].name, FONT3, 25,
                     i * font3Height + initialOffset, BLACK, WHITE);
  }
...
void drawCursorHandler(const uint8_t loc, const uint8_t total) {
  ILI9341_DrawFilledRectangleCoord(0, initialOffset, 25,
                                   total * font3Height + initialOffset, BLACK);
  ILI9341_DrawChar('>', FONT3, 10, initialOffset + font3Height * loc, BLACK,
                   WHITE);
}
...
void savePin(char *pin) {
  ...
  uint8_t buffer[4] = {savedPin[0] - '0', savedPin[1] - '0', savedPin[2] - '0',
                       savedPin[3] - '0'};
  writePincode(buffer);
}

void handleSelectPin() { ++pinIndex; }
...
char *getIP() {
  sprintf(IP, "%03d.%03d.%03d.%03d", IPTESTARRAY[0], IPTESTARRAY[1],
          IPTESTARRAY[2], IPTESTARRAY[3]);
  return IP;
}
